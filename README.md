...
The app can run offline thanks to a service worker generated by
[`vite-plugin-pwa`](https://github.com/vite-pwa/vite-plugin-pwa). Install the
plugin as a development dependency and Vite will build a service worker and use
`manifest.webmanifest` from the `public/` folder. The manifest refers to
`vite.svg` as a generic app icon, but you can replace it with your own
production icons:
